...
func (worker *Worker) Run(ctx context.Context, request RunRequest) (RunResult, error) {
	...
	if request.RequestID == "" || request.ArtifactID == "" {
		return RunResult{}, ErrWorkerInvalid
	}
	...
	if err := evalScript(quickContext, `globalThis.SourceServerExtensions = { register: function(value) { globalThis.__tracking_registered_extension = value; } }; globalThis.Scanning = { error: function(value) { return value; } };`); err != nil {
		return RunResult{}, ErrExtensionFailure
	}
	...
	artifactLiteral := strconv.Quote(request.ArtifactID)
	fileNameLiteral := strconv.Quote(request.FileName)
	expression := fmt.Sprintf(`(async function() {
		try {
			const extension = globalThis.__tracking_registered_extension;
			if (!extension || extension.artifactId !== %s || extension.sourceKey !== %s
				|| (%s !== "" && extension.fileName !== %s)) {
				return JSON.stringify({ok:false,error:{code:"extension_failure",safeMessage:"extension identity mismatch"}});
			}
	...
	})()`, artifactLiteral, artifactLiteral, fileNameLiteral, fileNameLiteral, operationLiteral, inputLiteral)
